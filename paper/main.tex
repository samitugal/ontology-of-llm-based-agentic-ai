...
Ontolojinin modellediği alan oldukça dinamik ve hızlı gelişen bir alandır. Yeni etmen mimarileri, araç entegrasyonları ve koordinasyon desenleri ortaya çıktıkça ontolojinin de güncellenmesi gerekmektedir. Bu sebeple ontoloji genişletilebilir bir yapıda tasarlanmış olup gelecekte şu yönlerde sürdürülmesi planlanmaktadır: (i) eşleme kapsamının AutoGen ve diğer çerçevelere genişletilmesi; (ii) daha büyük ve çeşitli senaryolarla nicel kapsama değerlendirmesi; (iii) yerel kapama ve SWRL kuralları aracılığıyla tanımlı sınıf çıkarımlarının güçlendirilmesi; (iv) ontolojinin \texttt{w3id.org} gibi bir PURL hizmeti üzerinden, içerik müzakeresini (\textit{content negotiation}) destekleyen çözümlenebilir (\textit{dereferenceable}) bir kanonik IRI altında yayımlanması ve topluluk katkısına açılması. Ontoloji ve artefaktlar hâlihazırda Zenodo'da kalıcı olarak arşivlenmiş olup (DOI: 10.5281/zenodo.20571344), söz konusu adım arşivsel DOI'nin ötesinde çözümlenebilir bir ontoloji IRI'si sağlamayı amaçlamaktadır.
