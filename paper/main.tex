...
\hline
\textbf{Ontoloji / Yaklaşım} & \textbf{Hafıza} & \textbf{Araç} & \textbf{Oturum} & \textbf{Orkestrasyon /} & \textbf{OWL~2 +} & \textbf{Çerçeve-} \\
...
\hline
BDM etmen taksonomileri & \checkmark & \checkmark & K & \checkmark & -- & \checkmark \\
\hline
...
\begin{tikzpicture}[
  ...
  rel/.style={->, font=\scriptsize\ttfamily}
]
...
\draw[rel] (agent) -- (cap)     node[midway, sloped, above] {hasCapability};
\draw[rel] (agent) -- (mem)     node[midway, right]         {hasMemory};
\draw[rel] (agent) -- (cfg)     node[midway, sloped, above] {hasConfiguration};
\draw[rel] (agent) -- (role)    node[midway, above]         {hasRole};
\draw[rel] (agent) -- (session) node[midway, above]         {hasSession};
\draw[rel] (agent) -- (group)   node[midway, sloped, above] {memberOf};
\draw[rel] (agent) -- (tc)      node[midway, right]         {makesToolCall};
\draw[rel] (agent) -- (plan)    node[midway, sloped, above] {createsPlan};
\draw[rel] (tc)    -- (tool)    node[midway, right]         {invokesTool};
\draw[rel] (cap)   -- (tool)    node[midway, sloped, above] {usesTool};
\end{tikzpicture}
